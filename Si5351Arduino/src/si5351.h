   ...
   struct FastPLL
{
  ...
  Si5351(TwoWire *theWire = &Wire, uint8_t i2c_addr = SI5351_BUS_BASE_ADDR);
	...
	void reset(void);
	void initFastPLLtables(int32_t correction_base); //dodana
	...
	void set_pll2(uint64_t, enum si5351_pll);  //dodal za FM
	void output_enable_fast(uint8_t enable);
    /*void setCorrectionFastUSB(int dif); //dodana
    void setCorrectionFastLSB(int dif); //dodana
    void setCorrectionFastFM(int dif);*/  //dodana
	void writeFastPLL(const FastPLL &r); //dodana
	void setCorrectionFastUSB(int dif); //dodana
	void setCorrectionFastLSB(int dif); //dodana
	void setCorrectionFastFM(int dif);  //dodana
	...
	void set_correction2(int32_t, enum si5351_pll_input);  //dodal za FM
  ...
  void set_ref_freq(uint32_t, enum si5351_pll_input);
	...
	uint8_t si5351_write_bulk2(uint8_t, uint8_t, uint8_t *);   // dodal za FM
...
private:
	...
	//void buildFastTable(FastPLL *tbl, int slope, int32_t correction_base, int basecorr);
	void buildFastTableLSB(int32_t correction_base);
	void buildFastTableUSB(int32_t correction_base);
	void buildFastTableFM(int32_t correction_base);
  ...
  FastPLL pllTableUSB[DIF_SIZE];
  FastPLL pllTableLSB[DIF_SIZE];
  FastPLL pllTableFM[DIF_SIZE];
