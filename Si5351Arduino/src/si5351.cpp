...
Si5351::Si5351(TwoWire *theWire, uint8_t i2c_addr):
...
bool Si5351::init( uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr)
{
	...
	_wire->beginTransmission(i2c_bus_addr);
	uint8_t reg_val;
  reg_val = _wire->endTransmission();
...
void Si5351::set_pll2(uint64_t pll_freq, enum si5351_pll target_pll)
{
  struct Si5351RegSet pll_reg;
  ...
  //if(target_pll == SI5351_PLLA)
  //{
    si5351_write_bulk2(SI5351_PLLA_PARAMETERS + 4, i, params);  // prvi 4 byti so konstantni
		plla_freq = pll_freq;
  /*}
  else if(target_pll == SI5351_PLLB)
  {
...
void Si5351::set_correction2(int32_t corr, enum si5351_pll_input ref_osc)
{
	ref_correction[(uint8_t)ref_osc] = corr;

	// Recalculate and set PLL freqs based on correction value
	set_pll2(plla_freq, SI5351_PLLA);
	//set_pll(pllb_freq, SI5351_PLLB);
}
...
uint8_t Si5351::si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
 ...
 // dodam za FM
uint8_t Si5351::si5351_write_bulk2(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	_wire->beginTransmission(i2c_bus_addr);	
	_wire->write(addr);
...
uint8_t Si5351::si5351_read(uint8_t addr)
{
	...
	_wire->requestFrom(i2c_bus_addr, (uint8_t)1, (uint8_t)false);
...
/********************************************************************
   Build one table
********************************************************************/
/*void Si5351::buildFastTable(Si5351::FastPLL *tbl, int slope, int32_t correction_base, int basecorr)
{
  Si5351RegSet pll_reg;

  for (int dif = DIF_MIN; dif <= DIF_MAX; dif++)
  {
    int idx = dif - DIF_MIN;
    int32_t corr = correction_base + basecorr - dif * slope;
    // temporarily set correction
    ref_correction[(uint8_t)SI5351_PLL_INPUT_XO] = corr;

    // use your existing pll_calc ONCE during init
    pll_calc(SI5351_PLLA,
                    plla_freq,
                    &pll_reg,
                    corr,
                    0);
    // only bytes actually written by write_bulk2()
    tbl[idx].b0 = (uint8_t)(pll_reg.p1 & 0xFF);
    tbl[idx].b1 =
      (uint8_t)(((pll_reg.p3 >> 12) & 0xF0) |
      ((pll_reg.p2 >> 16) & 0x0F));

    tbl[idx].b2 = (uint8_t)((pll_reg.p2 >> 8) & 0xFF);
    tbl[idx].b3 = (uint8_t)(pll_reg.p2 & 0xFF);
  }
}*/
...
/********************************************************************
   LSB table
********************************************************************/
void Si5351::buildFastTableLSB(int32_t correction_base)
{
    Si5351RegSet pll_reg;
    ...
    for (int dif = DIF_MIN; dif <= DIF_MAX; dif++)
    {
        ...
        int32_t corr = correction_base + 369236 - dif * 139;

        pll_calc(SI5351_PLLA, plla_freq, &pll_reg, corr, 0);

       pllTableLSB[idx].r30 = (uint8_t)(pll_reg.p1 & 0xFF);

        pllTableLSB[idx].r31 =
            (uint8_t)(((pll_reg.p3 >> 12) & 0xF0) |
                      ((pll_reg.p2 >> 16) & 0x0F));

        pllTableLSB[idx].r32 = (uint8_t)((pll_reg.p2 >> 8) & 0xFF);
        pllTableLSB[idx].r33 = (uint8_t)(pll_reg.p2 & 0xFF);
    }
}
...
/********************************************************************
   USB table
********************************************************************/
void Si5351::buildFastTableUSB(int32_t correction_base)
{
    Si5351RegSet pll_reg;
    ...
    for (int dif = DIF_MIN; dif <= DIF_MAX; dif++)
    {
        ...
        pllTableUSB[idx].r30 = (uint8_t)(pll_reg.p1 & 0xFF);

        pllTableUSB[idx].r31 =
            (uint8_t)(((pll_reg.p3 >> 12) & 0xF0) |
                      ((pll_reg.p2 >> 16) & 0x0F));

        pllTableUSB[idx].r32 = (uint8_t)((pll_reg.p2 >> 8) & 0xFF);
        pllTableUSB[idx].r33 = (uint8_t)(pll_reg.p2 & 0xFF);
    }
}
...
/********************************************************************
   FM table
********************************************************************/
void Si5351::buildFastTableFM(int32_t correction_base)
{
    Si5351RegSet pll_reg;
    ...
    for (int dif = DIF_MIN; dif <= DIF_MAX; dif++)
    {
        ...
        int32_t corr = correction_base + 369236 + dif * 200;

        pll_calc(SI5351_PLLA, plla_freq, &pll_reg, corr, 0);

        pllTableFM[idx].r30 = (uint8_t)(pll_reg.p1 & 0xFF);

        pllTableFM[idx].r31 =
            (uint8_t)(((pll_reg.p3 >> 12) & 0xF0) |
                      ((pll_reg.p2 >> 16) & 0x0F));

        pllTableFM[idx].r32 = (uint8_t)((pll_reg.p2 >> 8) & 0xFF);
        pllTableFM[idx].r33 = (uint8_t)(pll_reg.p2 & 0xFF);
    }
}

/********************************************************************
   Call once after carrier frequency set_freq(...)
********************************************************************/
/*void Si5351::initFastPLLtables(int32_t correction_base)
{
    buildFastTable(pllTableLSB, 139, correction_base, 369236);
    buildFastTable(pllTableUSB, 136, correction_base, 369236);
    buildFastTable(pllTableFM,  200, correction_base, 369236);
}*/
void Si5351::initFastPLLtables(int32_t correction_base)
{
    buildFastTableLSB(correction_base);
    buildFastTableUSB(correction_base);
    buildFastTableFM(correction_base);
}
/********************************************************************
   Very fast runtime write
********************************************************************/
void Si5351::writeFastPLL(const FastPLL &r)
{
    _wire->beginTransmission(i2c_bus_addr);
    _wire->write(30);     // register 30
    _wire->write(r.r30);
    _wire->write(r.r31);
    _wire->write(r.r32);
    _wire->write(r.r33);
...
/********************************************************************
   Runtime functions
********************************************************************/
void Si5351::setCorrectionFastUSB(int dif)
{
  dif = constrain(dif, DIF_MIN, DIF_MAX);
  writeFastPLL(pllTableUSB[dif - DIF_MIN]);
}

void Si5351::setCorrectionFastLSB(int dif)
{
  dif = constrain(dif, DIF_MIN, DIF_MAX);
  writeFastPLL(pllTableLSB[dif - DIF_MIN]);
}

void Si5351::setCorrectionFastFM(int dif)
{
    if (dif < DIF_MIN) dif = DIF_MIN;
    if (dif > DIF_MAX) dif = DIF_MAX;
    writeFastPLL(pllTableFM[dif - DIF_MIN]);
}

void Si5351::output_enable_fast(uint8_t enable)
{
    if(enable)
        oe_state &= ~(1 << 1);   // enable CLK1
    else
        oe_state |= (1 << 1);    // disable CLK1

    _wire->beginTransmission(i2c_bus_addr);
    _wire->write(SI5351_OUTPUT_ENABLE_CTRL);
    _wire->write(oe_state);
    _wire->endTransmission();
}