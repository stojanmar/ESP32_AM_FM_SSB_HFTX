 ...
 * Ver09 poboljšano delovanje po celem HF področju uvedem difgain
 */
 /********************************************************************
...
volatile int dif = 0;
volatile float difgain = 1.0f;  //will decrese with frequency selected
  ...
  server.on("/get", HTTP_GET, [] (AsyncWebServerRequest *request) {
    ...
    if (request->hasParam(PARAM_INPUT_1)) {
      ...
      frekvin = frekvenca.toInt(); // morda funkcija atol() tudi dela
        ...
        difgain = (float) (7100000 / frekvin);  // for complete HF range
        }
        else if (txmode == 3) {
        si5351.set_correction(CORRECTION, SI5351_PLL_INPUT_XO);   // zamika frekvenco?
...
void loop() {
  ...
  //anvhod34 = anvhod34>>1;  //8 bitno
    ...
    dif = (int)(df * difgain); // deviacija frekvence od nosilca 
  ...
  case 4: {  //FM modulacija se spremeni v sampling 5kHz
   ...
   //anvhod34 = anvhod34>>1;  //8 bitno
    ...
    dif = (int) (adc * difgain * 8.0f);  // 8.0f adapt if needed
